// Módulo de Referidos — preparado para separación progresiva.
// Se mantiene sin activar hasta adaptar sus dependencias (persist, render, esc, todayISO).
(function(){
/* ---------- Programa de Referidos ---------- */
  ...
  function abrirNuevoReferido(){
    ...
    document.body.appendChild(o); const close=()=>o.remove(); document.getElementById('ref-close').onclick=close; document.getElementById('ref-cancel').onclick=close;
    document.getElementById('ref-save').onclick=async()=>{const a=document.getElementById('ref-r').value.trim(),b=document.getElementById('ref-n').value.trim();if(!a||!b){showToast('El referidor y el nuevo cliente son obligatorios.');return;} referidos=[{id:String(Date.now()),referidor:a,telefonoReferidor:document.getElementById('ref-tr').value.trim(),referido:b,telefonoReferido:document.getElementById('ref-tn').value.trim(),fecha:document.getElementById('ref-f').value,servicio:document.getElementById('ref-s').value.trim(),credito:parseFloat(document.getElementById('ref-c').value)||0,estado:document.getElementById('ref-e').value},...referidos]; if(await persist()){close();showToast('Referido registrado.');}};
  }
  window.CRMReferidos = {
    renderReferidos,
    abrirNuevoReferido
